...
## おまけ: MTGデッキシミュレーター(`mtg.html`)

同じリポジトリに、マジック:ザ・ギャザリングのデッキシミュレーターを別ページとして同梱しています。
デプロイ先URLの `/mtg.html`(開発時は http://localhost:5173/mtg.html )で開けます。

- カードセットは **M10(基本セット2010)限定**。カードデータ・画像は初回表示時に
  [Scryfall API](https://scryfall.com/docs/api) から取得し、localStorageに7日間キャッシュします
- 白・青・黒・赤・緑の**単色リミテッド用40枚デッキ(呪文23枚+基本土地17枚)を5つ**収録
- デッキリスト閲覧(タップで拡大表示)と、ドローシミュレーション
  (シャッフル/初手7枚/マリガン/キープ/ドロー/場に出す/タップ/墓地)に対応
- スマートフォンのブラウザ向けにレイアウトを最適化(対戦機能は未実装)

ソースコードは `src/mtg/` 以下にあり、Ruby Runner本体とはエントリーポイント(`mtg.html`)ごと
分離されています。
